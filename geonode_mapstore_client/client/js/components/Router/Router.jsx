...
function ThemeLoader({ themeCfg, loaderComponent, children }) {
  ...
  return (
    <Theme {...themeCfg} onLoad={() => setLoading(false)}>
...
const Router = forwardRef(
  (
    ...
    const user = initialState?.defaultState?.security?.user?.username;
    if (user===undefined) {
    ...
    return (
      <>
        <ThemeLoader themeCfg={themeCfg} loaderComponent={loaderComponent}>
...
Router.propTypes = {
  ...
  pluginsConfig: PropTypes.oneOfType([PropTypes.array, PropTypes.object]),
