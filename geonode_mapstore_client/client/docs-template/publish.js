...
function buildNav(members) {
    let globalNav;
    let nav = '<h2><a href="index.html">GeoNode MapStore Client</a></h2>';
    const seen = {};
    const seenTutorials = {};

    nav += buildMemberNav(members.modules, 'Plugins', {}, linkto);
...
exports.publish = (taffyData, opts, tutorials) => {
    ...
    attachModuleSymbols( find({ longname: {left: 'plugin:'} }), members.modules );
    ...
    generate('GeoNode MapStore Client',
        packages.concat(
            [{
                kind: 'mainpage',
                readme: opts.readme,
    ...
    Object.keys(helper.longnameToUrl).forEach(longname => {
        ...
        if (myModules.length) {
            generate(`${myModules[0].name} Plugin`, myModules, helper.longnameToUrl[longname]);
        }
